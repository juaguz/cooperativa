...
@section ('title') {{$action}} servicio @stop

@section ('content')
<h1>{{$action}} Servicio</h1>



{{ Form::model(isset($servicio) ? $servicio : null,$form_data, array('role' => 'form')) }}
...
<div class="row">
    <div class="form-group col-md-4">
        {{ Form::label('id_penia', 'Peña') }}
        {{ Form::select('id_penia',$peniasCombo,null,array('class' => 'form-control select2')) }}
...
<script>
    $(function(){
        ...
        if(idPrestamo != ''){
            ...
            $.ajax({
                url:"{{ route('servicios.getTablaServicios') }}",
                type:"POST",
                data:{id_servicio:idPrestamo},
                beforeSend:function(){
